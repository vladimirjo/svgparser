...
class CritErr(Enum):
    ...
    ELEMENT_INVALID = "Invalid element."
...
class ErrorToken:
    def __init__(
        ...
        if intoken_pointer == -1:
            self.intoken_pointer = len(token.chars) - 1
        else:
...
class ErrorCollector:
    ...
    def add(
        ...
        if intoken_pointer < -1:
            raise ValueError("Values lesser than -1 are not allowed.")
        if intoken_pointer == -1:
            self.tokens.append(ErrorToken(token, error, len(token.chars) - 1, options))
        else:
            self.tokens.append(ErrorToken(token, error, intoken_pointer, options))
